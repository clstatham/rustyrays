...
impl Distribution2D {
    ...
    pub fn pdf(&self, p: Point2) -> F {
        let iu = I::clamp((p[0] * self.p_conditional_v[0].count() as F) as I, 0, self.p_conditional_v[0].count()  as I- 1) as S;
        let iv = I::clamp((p[1] * self.p_marginal.count() as F) as I, 0, self.p_marginal.count() as I - 1) as S;
